...
/*void enqueue(message m, client *client)//Met en file une commande
{
    sem_wait(&client->sem_file);//On attend que personne n'utilise la file

    m_node *node = (m_node*) malloc(sizeof(m_node));//On crée le noeud
    node->next = NULL;
    node->value = m;
    if (client->last != NULL)//On ajoute le noeud au client(file en liste chainée)
        client->last=client->last->next = node;
    else
        client->first = client->last = node;
    //On indique au thread qu'un élément est pret
    sem_post(&client->sem_element);
    //On libère la liste
    sem_post(&client->sem_file);
}*/
